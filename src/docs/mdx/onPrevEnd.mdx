...
# onPrevEnd
#### `onPrevEnd` will pass the new __current__ `item-object` and __current__ `page-index`:  
```
...
<Playground >
    <Carousel 
        ...
        itemsToShow={2}
        onPrevEnd={(currentItem, pageIndex) => alert(JSON.stringify({currentItem, pageIndex}))}
    >
